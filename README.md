...
`notify.maxTextChars`（默认 1800）现在是**每条消息**的预算，不是整条回复的上限：模型回复很长时，插件会按段落/行边界拆成多条消息依次发送（标题带 `(1/3)` 这种角标），全文送达而不会在末尾出现 `…`。钉钉对单条消息有长度上限，超了会整条拒收（不是只截断），所以「拆多条」是唯一能完整展示长回复的办法。代码围栏跨消息时会自动闭合再重开，两半都还是代码块。
